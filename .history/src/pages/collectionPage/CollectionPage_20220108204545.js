...
function CollectionPage() {
    ...
    return (
        <main className="container">
            ...
            <div className="row row-cols-1 row-cols-md-3 g-3 d-flex">
                {/* show loader if isLoading is true */}
                {isLoading && <div className="loader">{t("loader.loader")}</div>}
                ...
                {!isLoading &&
                    ...
                    filtredArts.map(({ id, pictureName, image, lifeAndCountry, painter, body, dimention, matherial, year }) => {
                        return (
                            <div key={id}>
                                <section className="art d-flex h-100 pt-3 px-3 align-items-stretch">
                                    <Art
                                        id={id}
                ...
                {!isLoading && !filtredArts.length && <div>
                    <p className="error-message">{t("title.1")}</p>
                </div>}
